...
public class BrenfordLaw
{
    public static int[] GetBenfordStatistics(string text)
    {
        var statistics = new int[10]; // Массив для хранения статистики цифр
        ...
        for (int i = 0; i < text.Length; i++)
        {
            ...
            {
                // Проверяем, является ли текущая цифра первой цифрой числа
                if (i == 0 || !char.IsDigit(text[i - 1]))
                {
                    int digit = text[i] - '0'; // Преобразуем символ в цифру
                    if (digit > 0 && digit < 10)
                    {
                        statistics[digit]++; // Увеличиваем счетчик для этой цифры
                    }
                }
            }
        }
    ...
    {
        for (int i = 1; i < numbers.Length; i++) // Начинаем с 1, так как 0 не используется в законе Бенфорда
        {
            Console.WriteLine($"Digit {i}: {numbers[i]}");
        }
    }

    // Пример использования
    public static void Brenford_law()
    {
        string tallestBuildings = @"
            Burj Khalifa 830
